...
TEST(ShapeParserTest, parser_rectangle) {
    ...
    std::stack<Shape*> results = sp.getResult();
...
TEST(ShapeParserTest, parser_empty_compoundShape) {
    ...
    ASSERT_EQ(1, sp.getResult().size());
    ASSERT_EQ("Compound Shape {}", sp.getResult().top()->info());
}
...
TEST(ShapeParserTest, parser_compoundShape_that_contains_rectangle) {
    ...
    ASSERT_EQ(1, results.size());
    ASSERT_EQ("Compound Shape {Rectangle (3.000, 4.000)}", results.top()->info());
}
...
TEST(ShapeParserTest, parser_compoundShape_that_contains_multi_simple_shapes) {
    ...
    ASSERT_EQ(1, results.size());
    ASSERT_EQ("Compound Shape {Rectangle (3.000, 4.000), Ellipse (4.200, 3.700), Triangle ([0.000, 0.000], [0.000, -3.000], [-4.000, 0.000])}", results.top()->info());
}
