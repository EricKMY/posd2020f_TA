...
void InfoVisitor::visit(Ellipse* ellipse){
    char info[100];
    sprintf(info, "Ellipse (%.3f, %.3f)", ellipse->semiMajorAxes(), ellipse->semiMinorAxes());
    _info = info;
}

void InfoVisitor::visit(Triangle* triangle){
    char info[100];
...
void InfoVisitor::visit(Rectangle* rectangle){
    char info[100];
    sprintf(info, "Rectangle (%.3f, %.3f)", rectangle->length(), rectangle->width());

    _info = info;
}
void InfoVisitor::visit(CompoundShape* compoundShape){
